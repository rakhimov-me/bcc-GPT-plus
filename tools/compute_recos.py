...
def benefit_credit_card(cid):
    ...
    online = ["Играем дома", "Смотрим дома", "Едим дома", "Кино"]
...
# ---------- карта «релевантных» категорий под продукт ----------
PRODUCT_TO_CATS = {
    # Тревел: транспорт и поездки
    "Карта для путешествий": {"Путешествия", "Отели", "Такси", "АЗС", "Авто"},
    # Премиум: повышенный кэшбэк
    "Премиальная карта": {"Кафе и рестораны", "Ювелирные украшения", "Косметика и Парфюмерия"},
    # Кредитная карта: категории, которые часто выбирают как «любимые» + онлайн
    "Кредитная карта": {"Играем дома", "Смотрим дома", "Едим дома", "Кино", "Развлечения", "Книги", "Подарки", "Кафе и рестораны"},
    # Остальные – как правило, не завязаны на категориях покупок (оценка будет чаще 0)
    "Обмен валют": set(),
    "Кредит наличными": {"Ремонт дома", "Мебель", "Медицина", "Ювелирные украшения", "Подарки"},
    "Депозит Мультивалютный": set(),
    "Депозит Сберегательный": set(),
    "Депозит Накопительный": set(),
    "Инвестиции": set(),
    "Золотые слитки": set(),
}

def temp_points_by_top4_categories(chosen_product: str, top4_categories: list[str]) -> int:
    """Счёт по совпадениям топ-категорий с релевантными для выбранного продукта."""
    relevant = PRODUCT_TO_CATS.get(chosen_product, set())
    if not relevant:
        # продукт не основан на покупках по категориям — ставим 0 как «временную» метрику
        return 0
    hits = sum(1 for c in top4_categories if c in relevant)
    if hits >= 4: return 20
    if hits == 3: return 15
    if hits == 2: return 10
    if hits == 1: return 5
...
# ---------- безопасное сохранение (боремся с занятым Excel) ----------
...
def safe_to_csv(df: pd.DataFrame, path: str, **kw):
    try:
        ...
        p = Path(path)
        fallback = p.with_name(f"{p.stem}_{ts}{p.suffix}")
        df.to_csv(fallback, **kw)
        print(f"[WARN] {path} занят другим процессом. Сохранил в: {fallback}")

# ---------- прогон ----------
rows, dbg = [], []
for _, r in clients.iterrows():
    cid = int(r["client_code"])
    ...
    # топ-4 категорий трат ЗА ПОСЛЕДНИЙ МЕСЯЦ
    cl_last = spend_lastm[spend_lastm.client_code == cid].sort_values("sum_spend_lastm", ascending=False)
    top4_cats = cl_last.category.head(4).tolist()

    # временная точность (по категориям vs выбранный продукт)
    temp_points = temp_points_by_top4_categories(prod, top4_cats)
    ...
    rows.append({
        ...
        "Точность": temp_points  # по твоему требованию: 4-я колонка, временная
    })

    dbg.append({
        "client_code": cid,
        "name": r["name"],
        **{f"rank{i+1}": p for i, (p, _) in enumerate(ranked[:4])},
        **{f"score_{p}": s for p, s in ranked},
        "top4_categories_last_month": ", ".join(top4_cats),
    })

# ---------- сохранение ----------
safe_to_csv(
    pd.DataFrame(rows).sort_values("client_code"),
    "recommendations.csv",
    index=False,
    encoding="utf-8-sig",
    sep=";",
...
safe_to_csv(
    pd.DataFrame(dbg).sort_values("client_code"),
    "top4_debug.csv",
    index=False,
    encoding="utf-8-sig",
    sep=";",
